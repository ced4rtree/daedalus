...
- [[#setup][Setup]]
  ...
  - [[#set-user-emacs-directory][Set ~user-emacs-directory~]]
...
This config is loaded into the read-only nix store, so emacs won't be able to
write to it. Instead, we'll have emacs write elsewhere.
