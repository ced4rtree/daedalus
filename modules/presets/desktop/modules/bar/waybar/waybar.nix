{ config, inputs, ... }: let
  windowManager = config.daedalus.windowManager;
in {
  flake.modules.nixos.waybar = { pkgs, ... }: {
    ...
    programs.waybar = {
      enable = true;
      package = config.flake.packages.${pkgs.stdenv.hostPlatform.system}.waybar;
    };
  };
  ...
  perSystem = { pkgs, lib, ... }: let
    ...
    waybarConf = pkgs.writeText "waybar-config.jsonc" (lib.generators.toJSON { } {
      layer = "top";
      position = "bottom";
      height = 10;

      modules-left = ["${windowManager}/workspaces" "${windowManager}/window"];
      modules-right = [
        "temperature"
        "network"
        "backlight"
