...
#+end_src
** Evil
The Extensible VI Layer for emacs is a great way to add vim keys to emacs
#+begin_src emacs-lisp
  (use-package evil
    :ensure t
    :init
...
#+end_src
*** Evil Collection
The easiest way to 'ubiquitize' evil keybindings
#+begin_src emacs-lisp
...
#+end_src
** General
A modern way to customize your keybindings that lets you use space as a prefix key
#+begin_src emacs-lisp
  (use-package general
    :ensure t
    :config
...
#+end_src
** Scrolling
Scroll down a line while maintaining the cursor position wherever it is
#+begin_src emacs-lisp
  (general-define-key
